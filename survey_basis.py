...
def _wall_points(df, parameters=None):
    ...
    try:
        from post_process.labels import label_mask
...
def yaw_matrix(yaw_deg):
    ...
    Note the convention: the stages align with ``xyz @ SURVEY_BASIS`` and rotate
    back out with ``xyz @ SURVEY_BASIS.T`` (see coordinate_util's docstring).
    The call chain hides this -- walls.py hands ``SURVEY_BASIS.T`` to
    point_axis_align, which itself applies ``.T``, so the two cancel. Getting
    this backwards yields a matrix that rotates the building the wrong way by
    twice the yaw, which still looks plausible but is not axis-aligned.
    """
