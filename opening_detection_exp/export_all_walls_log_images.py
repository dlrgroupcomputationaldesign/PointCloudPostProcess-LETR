...
OUTPUT_DIR = "opening_detection_exp"
FT_TO_M = 0.3048
LENGTH_BIN_M = 0.05
Z_BIN_M = 0.05
MARGIN_M = 0.25
...
CELL_PX = 4
# Colormap: "gray_r" matches the matplotlib panels (white=low, black=high).
CMAP = "gray_r"
...
def _save_clean_image(out_path, img, cmap=CMAP, cell_px=CELL_PX):
    ...
    upscale; nearest-neighbor keeps pixels crisp.
    """
    h, w = img.shape
    dpi = 100
...
def main() -> None:
    ...
    for d in walls_data:
        log_img = np.log1p(d["count_img"].astype(np.float64))
        out_path = out_dir / f"wall_{d['id']}_log.png"
        _save_clean_image(str(out_path), log_img)
        print(f"  wrote {out_path}  pts={d['pts_used']}  shape={log_img.shape}")
